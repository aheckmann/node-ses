 ...
 * @api Public
 **/
Email.prototype.send = async function (callback) {
  var invalid = this.validate();

  if (invalid) {
    console.log("Got invalid", invalid)
    return Promise.reject(invalid).asCallback(callback);
  }
    console.log("Got valid", invalid)
  ...
  return fetch(this.amazon, {
    method: 'post',
    body: JSON.stringify(data),
    headers: signedOpts.headers
  })
  .catch(err => this._processFetchError(err).asCallback(callback))
  .then(res  => this._processResponse(res).asCallback(callback, { spread: true }))


  //request(signedOpts, (err, res, data) => {
  //    this._processResponse(err, res, data, callback)
  //});
};

Email.prototype._processFetchError = function (err) {
 ...
 *
 */
Email.prototype._processResponse = function (res) {
  debug('received: %d %j', res && res.status || 0, res.body);

  console.log("before res.json()", res.status)
  const data = res.json();
  console.log("successfully called res.json()")
